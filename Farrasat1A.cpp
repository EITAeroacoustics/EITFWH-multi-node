...
// 放到 Farrasat1A.cpp 中
void Farrasat1A::CalTimeSignalBlocked(int BLOCK_T, int /*localRank*/, int /*localSize*/)
{
    //================= 计时变量 =================
    double t_src_local   = 0.0;  // CalSourceTerms 本地耗时
    double t_deriv_local = 0.0;  // InterpTimeDerivative 本地耗时
    double t_accum_local = 0.0;  // 分块推进累加 本地耗时
    double t_total_local = 0.0;  // 函数总耗时

    const double T0_all = MPI_Wtime();

    //================= 1) 源项与时间导数 =================
    const double T0_src = MPI_Wtime();
    CalSourceTerms();
    const double T1_src = MPI_Wtime();
    t_src_local = T1_src - T0_src;

    const double T0_der = MPI_Wtime();
    InterpTimeDerivative();
    const double T1_der = MPI_Wtime();
    t_deriv_local = T1_der - T0_der;

    //================= 2) 准备观测时间轴/输出缓冲 =================
    const size_t obsNum  = obsLocation.size();
    const size_t tObsNum = obsTimeNum;
    const size_t tSrcNum = srcTime.size();

    // 分配并清零观测信号
    pPrime.assign(tObsNum, std::vector<double>(obsNum, 0.0));

    // 观测时间步长（均匀）
    const double deltaT = (tObsNum > 1) ? (obsTime[1] - obsTime[0]) : 1.0;
    const double obsMin = obsTimeMin;
    const double obsMax = obsTimeMax;

    // 常量
    const vec3   M(-M0, 0.0, 0.0);
    const double MSqr = M.magSqr();
    const double pi   = 2.0 * acos(0.0);

    // 若是静止积分面，提前计算一次辐射几何；若移动则在每个源时间步更新
    if (!isMoving && tSrcNum > 0) {
        CalTimeDelayAtStep(0);
    }

    //================= 3) 分块推进 =================
    size_t processed = 0;
    const double T0_accum_all = MPI_Wtime();

    for (size_t t0 = 0; t0 < tSrcNum; t0 += (size_t)BLOCK_T)
    {
        const size_t t1 = std::min(t0 + (size_t)BLOCK_T, tSrcNum);
        const double Tb0 = MPI_Wtime();

        for (size_t it = t0; it < t1; ++it)
        {
            // 移动边界：每个源时刻更新几何；静止边界：前面已在 it=0 时计算
            if (isMoving || (it == 0 && t0 == 0)) {
                CalTimeDelayAtStep(it);
            }

            for (size_t j = 0; j < srcNum; ++j)
            {
                const double dS    = srcArea[j];
                const double Qn    = srcQn[it][j];
                const double QnDot = srcQnDot[it][j];
                const vec3&  Li    = srcLi[it][j];
                const vec3&  LiDot = srcLiDot[it][j];

                const double LM = dot(M, Li);

                for (size_t io = 0; io < obsNum; ++io)
                {
                    // 几何量
                    const double rMag = R[j][io];
                    const vec3&  rHat = Rhat[j][io];
                    const double td   = timeDelay[j][io];

                    // 时间推进到观测点的先进时间
                    const double advTime = srcTime[it] + td;

                    // 边界保护（通常 tMin = srcMin + min(td)，这里再做防御性检查）
                    if (advTime < obsMin || advTime > obsMax) continue;

                    // 方向余弦等
                    const double Mr    = dot(M, rHat);
                    const double Lr    = dot(Li,    rHat);
                    const double LrDot = dot(LiDot, rHat);

                    // 核函数（与 CalTimeSignal 保持一致）
                    const double denom1 = (1.0 - Mr);
                    const double inv_r  = 1.0 / rMag;

                    // S1~S5 与原始 CalTimeSignal 完全一致
                    const double S1 = QnDot * inv_r / (denom1 * denom1);
                    const double S2 = Qn * C0 * (Mr - MSqr) * (inv_r * inv_r) / (denom1 * denom1 * denom1);
                    const double S3 = LrDot / C0 * inv_r / (denom1 * denom1);
                    const double S4 = (Lr - LM) * (inv_r * inv_r) / (denom1 * denom1);
                    const double S5 = Lr * (Mr - MSqr) * (inv_r * inv_r) / (denom1 * denom1 * denom1);

                    const double pTotal = (S1 + S2 + S3 + S4 + S5) * dS / (4.0 * pi);

                    // 线性插值推进到离散观测时间点
                    const double fidx  = (advTime - obsMin) / deltaT;
                    const size_t iL    = (size_t)floor(fidx);

                    if (iL >= tObsNum) continue; // 超出右边界（极端舍入情况下）
                    const double wR = fidx - (double)iL;
                    const double wL = 1.0 - wR;

                    pPrime[iL][io] += wL * pTotal;
                    const size_t iR = iL + 1;
                    if (iR < tObsNum) pPrime[iR][io] += wR * pTotal;
                }
            }
        }

        const double Tb1 = MPI_Wtime();
        t_accum_local += (Tb1 - Tb0);
        processed += (t1 - t0);

        // 进度条（仅 master 打印）
        if (isMaster) {
            double pct = 100.0 * double(processed) / double(tSrcNum ? tSrcNum : 1);
            std::cout << "\rEIT-FWH:: Progress (blocked): " << std::setw(6) << std::fixed
                      << std::setprecision(2) << pct << " % " << std::flush;
        }
    }
    if (isMaster) std::cout << std::endl;

    const double T1_all = MPI_Wtime();
    t_total_local = T1_all - T0_all;

    //================= 4) 全局归约打印 =================
    auto reduce_and_print = [&](const char* title, double local_val) {
        double sum=0.0, minv=0.0, maxv=0.0;
        MPI_Reduce(&local_val, &sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&local_val, &minv, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
        MPI_Reduce(&local_val, &maxv, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        if (isMaster) {
            int world = 1; MPI_Comm_size(MPI_COMM_WORLD, &world);
            std::cout << "*************************************************\n";
            std::cout << title << " (s): avg=" << std::fixed << std::setprecision(2)
                      << (sum / world) << "  min=" << minv << "  max=" << maxv << "\n";
        }
    };

    reduce_and_print("Calculate 3D-Time Source term Time cost", t_src_local);
    reduce_and_print("InterpTimeDerivative Time cost",          t_deriv_local);
    reduce_and_print("Block accumulation Time cost",             t_accum_local);
    reduce_and_print("CalTimeSignalBlocked Total Time cost",     t_total_local);
...
void Farrasat1A::CalFreqSpectra2D()
{
	...
	const complex<double> I(0., 1.);//complex I
	...
	for (int ik = 0; ik < srcFrequencyNum; ik++)
	{
		...
		for (int ip = 0; ip < srcNum2D; ip++)
		{
			...
			for (int io = 0; io < obsLocation.size(); io++)//60
			{
